...
def get_args():
    ...
    parser.add_argument('--best-list',default=3 , type=int, help='number of best translations we choose')
    parser.add_argument('--diversity',default=1.0 , type=float, help='parameter to control the diversity of generated sentences')
...
def main(args):
    ...
    if (args.best_list> args.beam_size):
        args.best_list = args.beam_size
    ...
    for i, sample in enumerate(progress_bar):
        ...
        #          log_probs[i, :, j] = log_probs[i, :, j] - diversity * j
        ...
        for i in range(batch_size):
            for j in range(args.beam_size):
                ...
                #print(f"i:{i},j:{j},best_log_p:{best_log_p}")
        ...
        best_list = args.best_list
        best_sents = []

        for search in searches:
            best_n_sents = search.get_n_best(args.best_list)
            for sent in best_n_sents :
                best_sents.append(sent[1].sequence[1:].cpu())
        ...
        for ii, sent in enumerate(output_sentences):
            sent_id = int(sample['id'].data[int(ii/best_list)])
            if sent_id not in all_hyps:
                all_hyps[sent_id] = [sent]
            else:
                all_hyps[sent_id].append(sent)
